...
unsafe impl IntoPolicy for PROCESS_MITIGATION_USER_SHADOW_STACK_POLICY {
    type Policy = Self;
    fn into_policy(self) -> (process::mitigation::Policy, Self::Policy) { (process::UserShadowStackPolicy, self) }
}

unsafe impl IntoPolicy for UserShadowStackPolicy {
    type Policy = u32; // XXX
    fn into_policy(self) -> (process::mitigation::Policy, Self::Policy) { (process::UserShadowStackPolicy, PROCESS_MITIGATION_USER_SHADOW_STACK_POLICY::from(self).Flags) }
}
