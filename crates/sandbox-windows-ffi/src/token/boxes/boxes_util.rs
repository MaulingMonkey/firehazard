...
#[track_caller] pub unsafe fn assert_valid_after_header_slice<T, U, A: Allocator>(cbs: &CBoxSized<T, A>, ptr: *mut U, count: impl TryInto<usize>) -> &[U] {
    ...
    assert!(p + size_of::<T>() <= pslice,   "slice was expected to trail header in the same buffer");
    assert!(pslice <= pend,                 "slice was expected to trail header in the same buffer");
