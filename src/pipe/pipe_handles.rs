...
//pl io::Read   for &'_ pipe::DuplexNN  { fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> { unsafe { Ok(read_file(*self, buf, None).map(usize::from32)?) } } } // deadlock bait, do not implement - see "Quirks: Serialized I/O" rant on `mod pipe::named`
...
//pl io::Write  for &'_ pipe::DuplexNN  { fn write(&mut self, buf: &[u8]) -> io::Result<usize> { unsafe { Ok(write_file(*self, buf, None).map(usize::from32)?) } } fn flush(&mut self) -> io::Result<()> { Ok(()) } } // deadlock bait, do not implement - see "Quirks: Serialized I/O" rant on `mod pipe::named`
