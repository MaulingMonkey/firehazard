...
pub fn open_thread_token(thread: impl AsRef<crate::thread::Handle>, desired_access: impl Into<crate::token::AccessRights>, open_as_self: bool) -> Result<crate::token::Handle, crate::error::LastError> {
    let mut h = std::ptr::null_mut();
    let success = 0 != unsafe { winapi::um::processthreadsapi::OpenThreadToken(thread.as_ref().as_handle(), desired_access.into().into(), open_as_self as _, &mut h) };
