...
/// &nbsp;
pub fn adjust_token_privileges_enable_if(token: &token::Handle, mut cond: impl FnMut(privilege::Luid) -> bool) -> Result<(), Error> {
...
/// **Disabled privileges can be re-enabled** - this guards against accidents, not malware!<br>
/// &nbsp;
pub fn adjust_token_privileges_disable_if(token: &token::Handle, mut cond: impl FnMut(privilege::Luid) -> bool) -> Result<(), Error> {
    adjust_token_privileges_each(token, move |p| {
        let prev = p.attributes & SE_PRIVILEGE_ENABLED;
...
/// &nbsp;
pub fn adjust_token_privileges_retain_if(token: &token::Handle, mut cond: impl FnMut(privilege::Luid) -> bool) -> Result<(), Error> {
    adjust_token_privileges_each(token, move |p| {
        let prev = p.attributes & SE_PRIVILEGE_ENABLED;
...
/// Discarded privileges cannot be reapplied.<br>
/// &nbsp;
pub fn adjust_token_privileges_remove_if(token: &token::Handle, mut cond: impl FnMut(privilege::Luid) -> bool) -> Result<(), Error> {
    adjust_token_privileges_each(token, move |p| {
        let prev = p.attributes & SE_PRIVILEGE_ENABLED;
...
fn adjust_token_privileges_each(token: &token::Handle, mut adjust_attributes: impl FnMut(&mut privilege::LuidAndAttributes) -> bool) -> Result<(), Error> {
...
fn _adjust_token_privileges_disable_all(token: &token::Handle) -> Result<(), Error> {
...
unsafe fn adjust_token_privileges(token: &token::Handle, disable_all_privileges: bool, new_state: Option<&mut token::BoxTokenPrivileges>, _previous_state: Option<Infallible>, _return_length: Option<Infallible>) -> Result<(), Error> {
