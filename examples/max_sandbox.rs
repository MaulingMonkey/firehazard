...
fn run(context: &Context, target: Target) {
    ...
    let permissive = unsafe { duplicate_token_ex(&permissive, token::ALL_ACCESS, None, security::Impersonation, token::Impersonation) }.unwrap(); // primary -> impersonation token
