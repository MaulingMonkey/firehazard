...
#[repr(transparent)] pub struct CBox<T, D: Deallocator = ProcessHeapAllocFree>(*mut T, PhantomData<D>);
...
impl<T       , D: Deallocator> Drop     for CBox<T, D> { fn drop(&mut self) { unsafe { D::free(self.0) } } }
impl<T       , D: Deallocator> Deref    for CBox<T, D> { fn deref    (&    self) -> &    T { unsafe { &    *self.0 } } type Target = T; }
impl<T       , D: Deallocator> DerefMut for CBox<T, D> { fn deref_mut(&mut self) -> &mut T { unsafe { &mut *self.0 } } }
impl<T       , D: Deallocator> From<CBoxSized<T, D>> for CBox<T, D> { fn from(mut cbs: CBoxSized<T, D>) -> Self { let r = Self(cbs.as_mut_ptr(), PhantomData); forget(cbs); r } }
