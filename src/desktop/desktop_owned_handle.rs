...
impl Drop           for OwnedHandle { fn drop(&mut self) { assert!(self.0.is_null() || (0 != unsafe { CloseDesktop(self.0) }), "CloseDesktop({:?}) failed with GetLastError()={:?}", self.0, LastError::get()); } }
