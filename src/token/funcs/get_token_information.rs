...
pub fn user(token: &token::Handle) -> Result<BoxTokenUser, Error> { unsafe { Ok(BoxTokenUser::from_raw(raw_bytes(token, TokenUser)?)) } }
...
pub fn groups(token: &token::Handle) -> Result<BoxTokenGroups, Error> { unsafe { Ok(BoxTokenGroups::from_raw(raw_bytes(token, TokenGroups)?)) } }
...
pub fn privileges(token: &token::Handle) -> Result<BoxTokenPrivileges, Error> { unsafe { Ok(BoxTokenPrivileges::from_raw(raw_bytes(token, TokenPrivileges)?)) } }
...
pub fn owner(token: &token::Handle) -> Result<BoxTokenOwner, Error> { unsafe { Ok(BoxTokenOwner::from_raw(raw_bytes(token, TokenOwner)?)) } }
...
pub fn primary_group(token: &token::Handle) -> Result<BoxTokenPrimaryGroup, Error> { unsafe { Ok(BoxTokenPrimaryGroup::from_raw(raw_bytes(token, TokenPrimaryGroup)?)) } }
...
pub fn default_dacl(token: &token::Handle) -> Result<BoxTokenDefaultDacl, Error> { unsafe { Ok(BoxTokenDefaultDacl::from_raw(raw_bytes(token, TokenDefaultDacl)?)) } }
...
pub fn source(token: &token::Handle) -> Result<Source, Error> { unsafe { raw_fixed(token, TokenSource) } }
...
pub fn r#type(token: &token::Handle) -> Result<token::Type, Error> { unsafe { raw_fixed(token, TokenType) } }
...
pub fn impersonation_level(token: &token::Handle) -> Result<SECURITY_IMPERSONATION_LEVEL, Error> {
...
pub fn statistics(token: &token::Handle) -> Result<TOKEN_STATISTICS, Error> { unsafe { raw_fixed(token, TokenStatistics) } }
...
pub fn session_id(token: &token::Handle) -> Result<u32, Error> { unsafe { raw_fixed(token, TokenSessionId) } }
...
pub fn groups_and_privileges(token: &token::Handle) -> Result<BoxTokenGroupsAndPrivileges, Error> { unsafe { Ok(BoxTokenGroupsAndPrivileges::from_raw(raw_bytes(token, TokenGroupsAndPrivileges)?)) } }
...
pub fn sandbox_inert(token: &token::Handle) -> Result<bool, Error> { unsafe { raw_bool(token, TokenSandBoxInert) } }
...
pub fn origin(token: &token::Handle) -> Result<TOKEN_ORIGIN, Error> { unsafe { raw_fixed(token, TokenOrigin) } }
...
pub fn elevation_type(token: &token::Handle) -> Result<token::ElevationType, Error> { unsafe { raw_fixed(token, TokenElevationType) } }
...
pub fn linked_token(token: &token::Handle) -> Result<TOKEN_LINKED_TOKEN, Error> { unsafe { raw_fixed(token, TokenLinkedToken) } }
...
pub fn elevation(token: &token::Handle) -> Result<token::Elevation, Error> { unsafe { raw_fixed(token, TokenElevation) } }
/// \[[docs.microsoft.com](https://docs.microsoft.com/en-us/windows/win32/api/securitybaseapi/nf-securitybaseapi-gettokeninformation)\] `GetTokenInformation(self, TokenElevation, ...)`
pub fn is_elevated(token: &token::Handle) -> Result<bool, Error> { elevation(token).map(|e| !!e.token_is_elevated) }
...
pub fn has_restrictions(token: &token::Handle) -> Result<bool, Error> { unsafe { raw_bool(token, TokenHasRestrictions) } }
...
pub fn access_information(token: &token::Handle) -> Result<impl Deref<Target=TOKEN_ACCESS_INFORMATION>, Error> { unsafe { raw_header(token, TokenAccessInformation) } }
...
pub fn virtualization_allowed(token: &token::Handle) -> Result<bool, Error> { unsafe { raw_bool(token, TokenVirtualizationAllowed) } }
...
pub fn virtualization_enabled(token: &token::Handle) -> Result<bool, Error> { unsafe { raw_bool(token, TokenVirtualizationEnabled) } }
...
pub fn integrity_level(token: &token::Handle) -> Result<BoxTokenMandatoryLabel, Error> { unsafe { Ok(BoxTokenMandatoryLabel::from_raw(raw_bytes(token, TokenIntegrityLevel)?)) } }
...
pub fn ui_access(token: &token::Handle) -> Result<bool, Error> { unsafe { raw_bool(token, TokenUIAccess) } }
...
pub fn mandatory_policy(token: &token::Handle) -> Result<token::MandatoryPolicy, Error> { unsafe { raw_fixed(token, TokenMandatoryPolicy) } }
...
pub fn logon_sid(token: &token::Handle) -> Result<BoxTokenGroups, Error> { unsafe { Ok(BoxTokenGroups::from_raw(raw_bytes(token, TokenLogonSid)?)) } }
...
pub fn is_app_container(token: &token::Handle) -> Result<bool, Error> { unsafe { raw_bool(token, TokenIsAppContainer) } }
...
pub fn capabilities(token: &token::Handle) -> Result<BoxTokenGroups, Error> { unsafe { Ok(BoxTokenGroups::from_raw(raw_bytes(token, TokenCapabilities)?)) } }
...
pub fn app_container_sid(token: &token::Handle) -> Result<BoxTokenAppcontainerInformation, Error> { unsafe { Ok(BoxTokenAppcontainerInformation::from_raw(raw_bytes(token, TokenAppContainerSid)?)) } }
...
pub fn app_container_number(token: &token::Handle) -> Result<u32, Error> { unsafe { raw_fixed(token, TokenAppContainerNumber) } }
...
pub fn user_claim_attributes(token: &token::Handle) -> Result<impl Deref<Target=CLAIM_SECURITY_ATTRIBUTES_INFORMATION>, Error> { unsafe { raw_header(token, TokenUserClaimAttributes) } }
...
pub fn device_claim_attributes(token: &token::Handle) -> Result<impl Deref<Target=CLAIM_SECURITY_ATTRIBUTES_INFORMATION>, Error> { unsafe { raw_header(token, TokenDeviceClaimAttributes) } }
...
pub fn device_groups(token: &token::Handle) -> Result<BoxTokenGroups, Error> { unsafe { Ok(BoxTokenGroups::from_raw(raw_bytes(token, TokenDeviceGroups)?)) } }
...
/// ### Errors
/// *   `ERROR_INVALID_PARAMETER`   If `self` is a primary token instead of an impersonation token?
pub fn restricted_device_groups(token: &token::Handle) -> Result<BoxTokenGroups, Error> {
...
unsafe fn raw_len(token: &token::Handle, class: TOKEN_INFORMATION_CLASS) -> Result<u32, Error> {
    let mut size = 0;
...
/// ### Safety
/// *   `class` might need to be a valid token information class?
unsafe fn raw_bytes(token: &token::Handle, class: TOKEN_INFORMATION_CLASS) -> Result<Box<[u8]>, Error> {
    let mut size = 0;
...
unsafe fn raw_fixed<R: Copy>(token: &token::Handle, class: TOKEN_INFORMATION_CLASS) -> Result<R, Error> {
    let mut size = 0;
...
unsafe fn raw_bool(token: &token::Handle, class: TOKEN_INFORMATION_CLASS) -> Result<bool, Error> {
    let mut size = 0;
    let mut result = 0u32;
...
/// *   `class` might need to be a valid token information class?
/// *   `R` should probably be bytemuck::Zeroable or equivalent
unsafe fn raw_header<H: Copy>(token: &token::Handle, class: TOKEN_INFORMATION_CLASS) -> Result<impl Deref<Target = H>, Error> {
