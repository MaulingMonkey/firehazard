...
pub fn create<'a, 'b: 'a>(
    ...
    let security_attributes = security_attributes.into();
    let security_attributes = security_attributes.map_or(null(), |sa| sa) as *mut _;

    let handle = match name.as_narrow_or_wide() {
        NarrowOrWideCStrNonNull::Narrow(name) => unsafe { winapi::um::winbase::CreateNamedPipeA(
            name.as_cstr(),
            open_mode,
            pipe_mode,
            max_instances,
        ...
        NarrowOrWideCStrNonNull::Wide(name) => unsafe { winapi::um::namedpipeapi::CreateNamedPipeW(
            name.as_cstr(),
            open_mode,
            pipe_mode,
            max_instances,
