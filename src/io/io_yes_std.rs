...
// io::sync::OwnedFile → std::fs::File should be sound since the former currently forbids `FILE_FLAG_OVERLAPPED`.
impl From<io::sync::OwnedFile> for std::fs::File { fn from(file: io::sync::OwnedFile) -> Self { unsafe { std::fs::File::from_raw_handle(file.into_handle()) } } }
