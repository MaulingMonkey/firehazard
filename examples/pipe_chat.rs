...
fn client(mut args: std::env::Args) {
    ...
    let server_to_client = unsafe { pipe::ReaderNN::from_raw_nn(create_file_w(SERVER_TO_CLIENT_PIPE_NAME, GENERIC_READ,  0, None, OPEN_EXISTING, 0, None).unwrap().into_handle_nn()) };
    let client_to_server = unsafe { pipe::WriterNN::from_raw_nn(create_file_w(CLIENT_TO_SERVER_PIPE_NAME, GENERIC_WRITE, 0, None, OPEN_EXISTING, 0, None).unwrap().into_handle_nn()) };
    thread::scope(|s|{
