...
/// ```
///
pub fn convert_sid_to_string_sid_a(sid: &sid::Value) -> Option<alloc::CString<u8, alloc::LocalAllocFree>> {
...
/// ```
///
pub fn convert_sid_to_string_sid_w(sid: &sid::Value) -> Option<alloc::CString<u16, alloc::LocalAllocFree>> {
    if sid.as_psid().is_null() { return None }
    let mut local_string = null_mut();
